...
@section('title', 'Compose Article')
@section('header_title', 'Create New Article')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="bg-white border border-[#111111]/15 p-6 space-y-5">
                <div>
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title') }}" required
                        ...
                        placeholder="e.g. Enterprise Cloud VPS Latency Benchmarks in 2026">
                </div>

                <div>
                    <label for="slug" class="block text-xs font-mono uppercase tracking-wider text-[#111111] mb-2 font-medium">Custom Slug (Optional)</label>
                ...
                <div>
                    ...
                    <textarea id="excerpt" name="excerpt" rows="3"
                        ...
                        placeholder="Brief 1-2 sentence lead text for cards and meta descriptions...">{{ old('excerpt') }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-white border border-[#111111]/15 p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="focus_keyword" name="focus_keyword" value="{{ old('focus_keyword') }}"
                            class="w-full px-3 py-1.5 text-xs font-mono bg-white border border-[#111111]/30 focus:outline-none focus:border-[#111111]"
                            placeholder="e.g. cloud vps latency">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="meta_description" name="meta_description" rows="2"
                        class="w-full px-3 py-1.5 text-xs font-mono bg-white border border-[#111111]/30 focus:outline-none focus:border-[#111111]"
                        placeholder="Up to 160 characters for SERP snippets...">{{ old('meta_description') }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="url" id="canonical_url" name="canonical_url" value="{{ old('canonical_url') }}"
                            class="w-full px-3 py-1.5 text-xs font-mono bg-white border border-[#111111]/30 focus:outline-none focus:border-[#111111]"
                            placeholder="https://securofi.tech/article/original-slug">
                    </div>
        ...
        <div class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-white border border-[#111111]/15 p-6 space-y-4">
                ...
                <div>
                    ...
                    <select id="status" name="status" required class="w-full px-3 py-2 text-xs font-mono bg-white border border-[#111111]/30 focus:outline-none focus:border-[#111111]">
                        ...
                        <option value="scheduled" {{ old('status') == 'scheduled' ? 'selected' : '' }}>Scheduled (Set Date)</option>
                    </select>
                </div>

                <div>
                    <label for="published_at" class="block text-xs font-mono uppercase tracking-wider text-[#111111] mb-1">Publication Timestamp</label>
            ...
            <div class="bg-white border border-[#111111]/15 p-6 space-y-3">
                <label for="category_id" class="block text-xs font-mono uppercase tracking-wider text-[#111111] font-bold">Taxonomy Category *</label>
            ...
            <div class="bg-white border border-[#111111]/15 p-6 space-y-4">
                ...
                <div>
                    <label for="featured_image_file" class="block text-xs font-mono text-[#808080] mb-1">Direct File Upload</label>
                ...
                <div>
                    ...
                    <input type="url" id="featured_image" name="featured_image" value="{{ old('featured_image') }}"
                        class="w-full px-3 py-1.5 text-xs font-mono bg-white border border-[#111111]/30 focus:outline-none focus:border-[#111111]"
                        placeholder="https://picsum.photos/seed/article/800/500">
                </div>
            </div>
            ...
            <div class="bg-white border border-[#111111]/15 p-6 space-y-4">
                ...
                <div>
                    ...
                    <select id="country_rule" name="country_rule" onchange="toggleArticleCountryBox(this.value)"
                        class="w-full px-3 py-1.5 text-xs font-mono bg-white border border-[#111111]/30 focus:outline-none focus:border-[#111111]">
                        <option value="all" {{ old('country_rule', 'all') === 'all' ? 'selected' : '' }}>All Countries (Global Public)</option>
                        <option value="include" {{ old('country_rule') === 'include' ? 'selected' : '' }}>Only Include Selected Countries</option>
                        <option value="exclude" {{ old('country_rule') === 'exclude' ? 'selected' : '' }}>Exclude / Block Selected Countries</option>
                    </select>
                </div>
                ...
                <div id="article-country-box" class="space-y-2 {{ old('country_rule', 'all') === 'all' ? 'hidden' : '' }}">
                    ...
                    <div>
                        ...
                        <textarea id="restriction_fallback_message" name="restriction_fallback_message" rows="2"
                            ...
                            placeholder="e.g. This guide applies exclusively to US regulatory filings.">{{ old('restriction_fallback_message') }}</textarea>
                    </div>
                </div>
                    ...
                    <div>
                        <select name="master_article_id" class="w-full px-2 py-1.5 text-xs font-mono bg-white border border-[#111111]/30 focus:outline-none focus:border-[#111111]">
                            ...
                            @foreach ($potentialMasters as $pm)
                                <option value="{{ $pm->id }}" {{ old('master_article_id') == $pm->id ? 'selected' : '' }}>
...
tinymce.init({
    ...
    setup: function (editor) {
        // Keep the hidden textarea in sync so Laravel validation sees content
...
// Helper: insert shortcode text into TinyMCE at cursor
...
const addedSlugs = new Set(); // track what custom tags have been chip-added
...
function addChip(name, tagId) {
    ...
    // If we have a resolved tag ID (existing tag), add a hidden checkbox-style input
    if (tagId) {
        ...
        // New tag — send to backend via AJAX
        ...
        .then(r => r.json())
        .then(data => {
            // Replace with the resolved server-side ID
            const hidden = document.createElement('input');
            hidden.type = 'hidden'; hidden.name = 'tags[]'; hidden.value = data.id;
            hidden.dataset.slug = slug;
            dynInputs.appendChild(hidden);
            // Also tick the checkbox in reserved list if present
            const cb = document.querySelector(`.reserved-tag-cb[value="${data.id}"]`);
            if (cb) cb.checked = true;
        })
        .catch(() => {
            // Silently fail — tag won't be attached, no UX disruption
            removeChip(slug);
        });
    }
}
...
function removeChip(slug) {
    ...
    // Un-tick reserved checkbox if applicable
...
tagInput.addEventListener('keydown', function (e) {
    if (e.key === 'Enter' || e.key === ',') {
        ...
        if (val) {
            // Check if it matches a reserved tag name
