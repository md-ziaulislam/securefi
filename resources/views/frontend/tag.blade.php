...
<div class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
    ...
    <header class="border-b border-neutral/20 pb-6 sm:pb-8 mb-8 sm:mb-12">
        <div class="flex items-center gap-2 mb-2">
            ...
            <span class="text-xs font-mono uppercase tracking-widest text-neutral">Tag Archive</span>
        </div>
        <h1 class="text-2xl sm:text-3xl font-bold tracking-tight text-primary">
            #{{ $tag->name }}
        </h1>
        <div class="mt-2 text-xs font-mono text-neutral">
            {{ $articles->total() }} {{ Str::plural('article', $articles->total()) }} tagged
        </div>
    </header>
    ...
    @else
        <div class="p-12 text-center border border-neutral/20 bg-secondary">
            <p class="text-base font-bold text-primary">No articles found with this tag.</p>
        </div>
    @endif
